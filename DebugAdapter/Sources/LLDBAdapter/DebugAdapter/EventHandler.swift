...
public protocol DebugAdapterEventHandler {
    ...
    /// `performDefaultHanding(forEvent:data:connection:)` to delegate to the default implementation for requests not handled.
    func handleEvent(_ event: String, data: Data, connection: DebugAdapterConnection)
...
public extension DebugAdapterEventHandler {
    func handleEvent(_ event: String, data: Data, connection: DebugAdapterConnection) {
        let handled = performDefaultHandling(forEvent: event, data: data, connection: connection)
        if !handled {
            print("DebugAdapter received unhandled event: \(event)")
        }
    }
    
    @discardableResult func performDefaultHandling(forEvent event: String, data: Data, connection: DebugAdapterConnection) -> Bool {
        do {
            switch event {
            ...
            print("DebugAdapter caught error while decoding event: \(error)")
        }
